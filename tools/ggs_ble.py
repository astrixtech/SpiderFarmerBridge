...
async def run(args):
    ...
    async with BleakClient(address, timeout=20.0) as client:
        ...
        try:
            # Bleak 0.x exposed get_services(); Bleak 1.x+ uses the .services cache.
            if hasattr(client, "get_services"):
                svcs = await client.get_services()  # type: ignore[attr-defined]
                uuids = [s.uuid for s in svcs]
            else:
                # .services may be None until the first service discovery completes.
                uuids = [s.uuid for s in (client.services or [])]
            if uuids and UUID_SVC_CUSTOM not in uuids:
                print("[WARN] Custom Service 0x00FF nicht explizit gefunden – fahre fort.")
        except Exception as e:
            # Service discovery is optional for this script; FF01/FF02 access will still be attempted.
            print(f"[WARN] Service check skipped ({type(e).__name__}: {e})")

        print("[BLE] Notifications abonnieren…")
        def handle_notify(_, data: bytes):
            # Windows consoles often default to cp1252; force UTF-8-safe printing.
            msg = pretty(data)
            try:
                print(f"[NOTIFY] {msg}")
            except UnicodeEncodeError:
                print(("[NOTIFY] " + msg).encode("utf-8", errors="replace").decode("utf-8", errors="replace"))
